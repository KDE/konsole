...
bool Template::Element::isValidFmt(const QString &fmt, Var::DataType type)
{
    switch (type) {
    case Var::DataType::String:
        return fmt.endsWith(QLatin1Char('s'));
    case Var::DataType::Number:
        return true; // regexp in parser takes care of it
    default:
        return false;
    }
}
...
Template::Template(const QString &text)
    : _text(text)
{
    ...
    if (!element.children.isEmpty()) {
        ...
        case Var::DataType::Vector:
            if (data.vec.isEmpty()) {
                ...
                warn(element,
                     QStringLiteral("the list entry data type (%1) is not supported in childrenless elements.").arg(data.vec.at(0).dataTypeAsString()));
            }
            break;
        case Var::DataType::Map:
